...
checks = pd.read_csv(sys.stdin).assign(
...
latest_month_count = checks.iloc[0].pipe(
    lambda x: x["month_int"] + (x["year_int"] * 12)
)

totals = (
    checks.loc[
        lambda df: (df["month_int"] + (df["year_int"] * 12))
        > (latest_month_count - 12 * 3)
    ]
    .groupby("month_dt")["totals"]
    .sum()
)
